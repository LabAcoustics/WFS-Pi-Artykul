...
\subsection{Synteza pola akustycznego}
...
Od wielkości systemu (liczby głośników) zależą możliwości reprodukcji pola akustycznego -- przy wększym systemie możliwe jest symulowanie wielu źródeł rozmieszczonych w~przestrzeni~\cite{Goebel2019}. Dużą barierą w~tworzeniu systemów pozwalających na efektywną syntezę pola akustycznego jest złożoność obliczeniowa -- sygnał musi być przetwarzany w~czasie rzeczywistym dla wielu kanałów jednocześnie. Z~tego względu
...
do obliczeń procesory graficzne (ang. \textit{Graphics Processing Unit} --- GPU) lub tworząc odpowiednie struktury jednostek obliczeniowych
w~układach programowalnych (ang. \textit{Field-Programmable Gate Array} --- FPGA). Rozwiązania takie zostały przedstawione i~porównane w~artykule~\cite{multiCore}.
Drugim aspektem są koszty systemu WFS, wysokie przy dużej liczbie głośników oraz wykorzystaniu jednostek komputerowych o~dużej mocy obliczeniowej. Jednym z~założeń projektu było wykonanie takiego systemu przy użyciu urządzeń ,,niskobudżetowych", dostępnych dla przeciętnego użytkownika, co pozwoliłoby na upowszechnienie tej technologii.
