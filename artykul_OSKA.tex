...
\titlePL{System syntezy pola akustycznego}
%\titleEN{Multichannel wave field synthesis system}
\titleEN{Wave field synthesis system}
...
\summaryPL{%
  ...
  pozwala na osiągnięcie wrażeń przestrzennych niezależnych od punktu odsłuchu, w przeciwieństwie do pozostałch systemów, gdzie poprawny odbiór wymaga zajęcia przez słuchacza ściśle określonego miejsca.
...
\summaryEN{
  ...
  listener's position, unlike in other systems where proper perception requires
  the listener to be in a specific position. Loudspeakers that are in the matrix,
  through the use of
  appropriate input signal processing, can form the shape of the acoustic
  ...
  with subsequent arrays of loudspeakers, for a better wave field synthesis
  capabilities.
}
...
Reprodukcja dźwięku jest bardzo szerokim zagadnieniem akustyki. Obecnie istnieje wiele technik pozwalających na odtworzenie panujących w~wybranym miejscu warunków akustycznych w zgoła innym otoczeniu. Złożoność
...
\textit{sweet spot})~\cite{Bassres2008How, Frank2017Exploring}.
...
przemieszczania się w~strefie odsłuchu~\cite{Boone1995Spatial}. Dzięki takiemu rozwiązaniu nie zostają
...
którego dotarła fala akustyczna wyemitowana przez wirtualne źródło dźwięku~\cite{Spors2008TheTheory}.
...
źródeł w każdym punkcie zdefiniowanej przestrzeni~\cite{snakowska}. Jest używana do
...
\subsection{Systemy syntezy pola akustycznego}

Od wielkości systemu (liczby głośników) zależą możliwości reprodukcji pola
akustycznego -- przy wększym systemie możliwe jest symulowanie wielu źródeł
rozmieszczonych w~przestrzeni~\cite{Goebel2019}. Dużą barierą w~tworzeniu
systemów pozwalających na efektywną syntezę pola akustycznego jest złożoność
obliczeniowa -- sygnał musi być przetwarzany w~czasie rzeczywistym dla wielu
kanałów jednocześnie. Z~tego względu
dobre rezultaty przynoszą rozwiązania wykorzystujące wiele jednostek
obliczeniowych.
Taką architekturę systemu można również osiągnąć wykorzystując
do obliczeń procesory graficzne (ang. \textit{Graphics Processing Unit} ---
GPU) lub tworząc odpowiednie struktury jednostek obliczeniowych
w~układach programowalnych (ang. \textit{Field-Programmable Gate Array} ---
FPGA). Rozwiązania takie zostały przedstawione i~porównane
w~artykule~\cite{multiCore}.

Drugim aspektem są koszty systemu WFS, wysokie przy dużej liczbie głośników
oraz wykorzystaniu jednostek komputerowych o~dużej mocy obliczeniowej. Jednym
z~założeń projektu było wykonanie takiego systemu przy użyciu urządzeń
,,niskobudżetowych", dostępnych dla przeciętnego użytkownika, co pozwoliłoby na
...
%Wyniki przedstawia rysunek~\ref{r:fem}, co pokazuje dużą zbieżność pomiędzy modelami.

\begin{figure}[!ht]
	\begin{subfigure}[b]{\textwidth}
  \centering
  \includegraphics[height=.41\textheight]{bitgraphics/real_500Hz_pl.png}
...
\begin{subfigure}[b]{\textwidth}
	\centering
  \includegraphics[height=.41\textheight]{bitgraphics/virtual_500Hz_pl.png}
