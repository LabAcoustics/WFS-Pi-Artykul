...
for s in range(5, 6):
    for f in [200]:
        ...
        for it in range(8):
            matrix1[it] = np.mean(file1.object('PXI1Slot2',
                                               f"PXI1Slot2/ai{it}").data[t1-mean_s:t1+mean_s])
            matrix1[it+8] = np.mean(file1.object('PXI1Slot6',
                                                 f"PXI1Slot6/ai{it}").data[t1-mean_s:t1+mean_s])
            matrix2[it] = np.mean(file2.object('PXI1Slot2',
                                               f"PXI1Slot2/ai{it}").data[t2-mean_s:t2+mean_s])
            matrix2[it+8] = np.mean(file2.object('PXI1Slot6',
                                                 f"PXI1Slot6/ai{it}").data[t2-mean_s:t2+mean_s])
